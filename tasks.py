...
# governed almost entirely by information flow (the communication topology),
...
def single_agent_baseline(instances: list[dict], clue_index: int = 0,
    ...
    Uses the same model and prompts (SYSTEM_PROMPT + VOTE_PROMPT) as the multi-agent
    experiment, with no messages received, so the only difference vs a multi-agent run
    is the absence of communication. One agent holds one private clue, so it cannot
    reliably solve the puzzle (expected chance is 1/|candidates|). This is the
    reference the topology runs are compared against.
    """
