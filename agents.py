...
class Agent:
    def __init__(self, agent_id: int, clue: str, candidates: list[str], question: str, client, backend: str, model: str,
        ...
        # Per-agent token/call counters, used to verify the fixed-budget assumption across topologies.
        self.input_tokens = 0
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 200) -> str:
        # No artificial pacing between calls: every sleep() below is a retry backoff, not scheduled throttling.
        max_retries = 8
        base_delay = 5.0
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_retryable:
                    ...
                    print(f"\n[Retryable API error] Agent {self.agent_id} hit traffic limit. Retrying in {delay:.2f}s...")
                    time.sleep(delay)
                else:
                    raise e
...
class MultiAgentSimulator:
    ...
    def _recovery(self, votes: dict) -> tuple[Optional[float], Optional[float]]:
        ...
        recovered correctly. Returns (mean across agents, best single agent): mean is
        the expected recovery for a randomly chosen aggregator, best is the best-case
        aggregator. Returns (None, None) for non-fragment tasks."""
    ...
    def _position_majority(self, votes: dict) -> tuple[str, float]:
        ...
        Per-agent transcription errors are independent, so this pools partial knowledge
        and cancels clerical noise: a fully-reachable network recovers ~all positions
        even when individual agents are sloppy. Returns the aggregated code and the mean
        per-position agreement (fraction of agents backing the winning character,
    ...
    def run(self) -> dict:
        ...
        # Aggregate per-run budget across all agents.
        ...
        # `log` is left out of this dict to keep metrics.csv lean; the runner
        # persists self.log separately to results/raw_logs/.
        return {
